...
import io.getsafenow.libraries.preferences.api.AppPreferencesStore
...
private val customGsnCallBaseUrlKey = stringPreferencesKey("gsnCallBaseUrl")
...
class DefaultAppPreferencesStore @Inject constructor(
    ...
    private val store = preferenceDataStoreFactory.create("getsafenow_preferences")
    ...
    override suspend fun setCustomElementCallBaseUrl(string: String?) {
        store.edit { prefs ->
            if (string != null) {
                prefs[customGsnCallBaseUrlKey] = string
            } else {
                prefs.remove(customGsnCallBaseUrlKey)
            }
        }
    }
...
private fun BuildMeta.defaultLogLevel(): LogLevel {
    return when (buildType) {
        ...
        BuildType.STAGING -> LogLevel.DEBUG
